...
pub fn derive_key(password: &[u8], salt: &[u8]) -> SafeString {
    const PBKDF2_ITERATIONS: NonZeroU32 = NonZeroU32::new(300_000_u32).unwrap();

    let mut result = SafeString::new(vec![0_u8; ALGORITHM.key_len()].into_boxed_slice());
    pbkdf2::derive(
...
pub fn encrypt(mut plaintext: SafeString, key: &[u8]) -> Option<(SafeString, aead::Nonce)> {
    ...
    let (nonce, tag) = aead_key
        .seal_in_place_separate_tag(aead::Aad::empty(), &mut plaintext)
        .ok()?;
    ...
    let mut result = SafeString::new(vec![0_u8; result_len].into_boxed_slice());
...
mod tests {
    ...
    fn encrypt_and_decrypt() {
        ...
        let message = SafeString::new(
            (*b"I set my ATM card's number to '0001' because I'm number one!").into(),
        );
